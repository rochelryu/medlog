     ...
     $query = new Mysql_query($bdd);
        if($type == 'view'){
            ...
            $donnees = array(
                'modes' => "VUE = VUE + :vue",
                'table' =>  sujet_offre,
                'where' => "ID_SUJ_O =:sujet AND MODE=:mode",
            );
...
// toute les postes envoyer
                $get_dataP = array(
                    'champs' => "ID_POST_SUJ_O, PROFIL ,COMMENTAIRE ,CREER_PAR , CREER_LE",
                    'table' => post_sujet_o,
                    'where' => "ID_SUJ_O = :sujet",
                    'tri' => "ORDER BY ID_POST_SUJ_O DESC"
                );
...
// Mise a jour des commentaire pour avoir cliquer dessus
            ...
            if(!empty($dataUpdate)){
                ...
                // Hydrating and Update value Setting Area
                $donnees = array(
                    'champs' => "ID_SUJ_O ,PROFIL ,COMMENTAIRE ,CREER_PAR ,CREER_LE",
                    'table' => post_sujet_o,
